...
package com.sellwild.prebid.renderingtestapp
...
import com.sellwild.prebid.*
import com.sellwild.prebid.api.mediation.MediationNativeAdUnit
import com.sellwild.prebid.renderingtestapp.plugplay.config.*
import com.sellwild.prebid.renderingtestapp.utils.BaseFragment
import com.sellwild.prebid.renderingtestapp.utils.ConfigurationViewSettings
import com.sellwild.prebid.renderingtestapp.utils.OpenRtbConfigs
import com.sellwild.prebid.renderingtestapp.widgets.EventCounterView
...
abstract class AdFragment : BaseFragment() {
    ...
    protected var refreshDelay = SellwildPrebid.AUTO_REFRESH_DELAY_MIN / 1000
    ...
    override fun initUi(view: View, savedInstanceState: Bundle?) {
        arguments?.let {
            ...
            SellwildPrebid.setStoredAuctionResponse(storedAuctionResponse)
            ...
            it.getString(ARGUMENT_ACCOUNT_ID)?.let { accountId ->
                ...
                SellwildPrebid.setPrebidServerAccountId(accountId)
            }
        }
    ...
    private fun setNoBidsAccountId(enable: Boolean) {
        if (enable) {
            SellwildPrebid.setPrebidServerAccountId(getString(R.string.prebid_account_id_prod_no_bids))
        } else {
            SellwildPrebid.setPrebidServerAccountId(getString(R.string.prebid_account_id_prod))
        }
    }
    ...
    protected fun configureOriginalPrebid() {
        val hostUrl = SellwildPrebid.getPrebidServerHost().hostUrl
        val host = Host.CUSTOM
        host.hostUrl = hostUrl
        SellwildPrebid.setPrebidServerAccountId(SellwildPrebid.getPrebidServerAccountId())
        SellwildPrebid.initializeSdk(requireContext(), "https://prebid-server-test-j.prebid.org/openrtb2/auction", null)
    }
