...
package com.sellwild.prebid.rendering.bidding.data.bid;

import static com.sellwild.prebid.rendering.utils.helpers.Utils.addValue;
...
import com.sellwild.prebid.rendering.models.openrtb.bidRequests.mapper.PluginRendererListMapper;
...
public class Prebid {
    ...
    public static JSONObject getJsonObjectForBidRequest(
        ...
        if (SellwildPrebid.isUseCacheForReportingWithRenderingApi() || config.isOriginalAdUnit()) {
        ...
        if(SellwildPrebid.getIncludeWinnersFlag()){
            Utils.addValue(targeting, "includewinners", "true");
        }

        if(SellwildPrebid.getIncludeBidderKeysFlag()){
    ...
    private static void setPluginRendererList(JSONObject prebid, AdUnitConfiguration adUnitConfiguration) {
        List<SellwildPrebidPluginRenderer> plugins = SellwildPrebidPluginRegister.getInstance().getRTBListOfRenderersFor(adUnitConfiguration);
        ...
        boolean isDefaultPluginRenderer = rendererList.get().size() == 1
                && rendererList.get().get(0).getName().equals(SellwildPrebidPluginRegister.PREBID_MOBILE_RENDERER_NAME);
    ...
    private static void addStoredAuctionResponse(JSONObject prebid) {
        final String storedAuctionResponse = SellwildPrebid.getStoredAuctionResponse();
    ...
    private static void addStoredBidResponse(JSONObject prebid) {
        final Map<String, String> storedBidResponseMap = SellwildPrebid.getStoredBidResponses();
