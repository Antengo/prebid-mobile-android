...
import com.sellwild.prebid.rendering.networking.tracking.ServerConnection;
...
public class StatusRequester implements Callable<String> {
    ...
    public static String makeRequest() {
        String statusUrl;
        String customStatusEndpointUrl = SellwildPrebid.getCustomStatusEndpoint();
        if (customStatusEndpointUrl != null) {
            statusUrl = customStatusEndpointUrl;
        } else {
            String url = SellwildPrebid.getPrebidServerHost().getHostUrl();
            if (url.contains("/openrtb2/auction")) {
                statusUrl = url.replace("/openrtb2/auction", "/status");
            } else {
                LogUtil.info("Prebid SDK can't build the /status endpoint. Please, provide the custom /status endpoint using SellwildPrebid.setCustomStatusEndpoint().");
                return null;
            }
        }
