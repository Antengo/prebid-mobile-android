...
package com.sellwild.prebid.rendering.networking;
...
import com.sellwild.prebid.rendering.networking.exception.BaseExceptionHolder;
...
public class BaseNetworkTask
    ...
    public GetUrlResult sendRequest(GetUrlParams param) throws Exception {
        if (param.url.isEmpty()) {
            LogUtil.error(TAG, "url is empty. Set url in SellwildPrebid (PrebidRenderingSettings).");
        }
    ...
    private URLConnection setHttpURLConnectionProperty(GetUrlParams param) throws Exception {
        ...
        connection.setConnectTimeout(SellwildPrebid.getTimeoutMillis());
        if (!(this instanceof FileDownloadTask)) {
            int timeout = SellwildPrebid.getTimeoutModified() ? SellwildPrebid.getTimeoutMillis() : SOCKET_TIMEOUT;
    ...
    private void setCustomHeadersIfAvailable(URLConnection connection) {
        if (!SellwildPrebid.getCustomHeaders().isEmpty()) {
            for (Map.Entry<String, String> customHeader : SellwildPrebid.getCustomHeaders().entrySet()) {
